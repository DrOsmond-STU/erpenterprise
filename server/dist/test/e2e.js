...
async function main() {
    ...
    ok(rec.status === 200 && [7, 11].includes(rec.body.checks.length) && rec.body.checks.every((k) => k.ok), 'rekonsiliasi sub-buku cocok', rec.body?.checks?.filter((k) => !k.ok) ?? rec.body);
    ...
    console.log('Penjualan & piutang');
    {
        ...
        const braket = prods.find((p) => p.sku === 'BRG-1108');
        const bearing = prods.find((p) => p.sku === 'BRG-2217');
        const stockCkr = (p) => p.stock.filter((x) => x.branch === 'CKR').reduce((t, x) => t + Number(x.onHand), 0);
        ...
        ok(so2.status === 201 && so2.body.status === 'menunggu' && so2.body.approvalReasons.some((r) => /plafon/.test(r)), 'pesanan melebihi sisa plafon → menunggu persetujuan', so2.body?.approvalReasons);
        ...
        ok(upd.status === 200 && upd.body.status === 'disetujui' && upd.body.timeline.some((t) => t.action === 'sales_order.rejected'), 'pesanan ditolak diubah lalu diajukan ulang → disetujui; linimasa tercatat', upd.body?.status);
        ...
        ok(soHeld.body.status === 'menunggu' && soHeld.body.approvalReasons.some((r) => /ditahan/.test(r)), 'pelanggan ditahan → pesanan kecil pun menunggu persetujuan', soHeld.body?.approvalReasons);
        ...
        const rules = (iss.body.journals ?? []).map((j) => j.rule).sort();
        ok(JSON.stringify(rules) === JSON.stringify(['SALES_COGS', 'SALES_INVOICE']), 'jurnal penjualan & HPP diposting otomatis', rules);
        const jSales = await call(`/ledger/journals/${iss.body.journals.find((j) => j.rule === 'SALES_INVOICE').id}`, { token: andi, branch: 'ALL' });
        const amt = (acc, side) => jSales.body.lines.filter((l) => l.account === acc).reduce((t, l) => t + l[side], 0);
        ok(jSales.body.status === 'posted' && amt('1-1200', 'debit') === iss.body.total && amt('4-1000', 'credit') === 10 * braket.price && amt('4-2000', 'credit') === 4_500_000 && amt('2-1400', 'credit') === iss.body.ppn, 'jurnal: Dr piutang total; Cr pendapatan barang, jasa, PPN keluaran', jSales.body.lines);
        const prods2 = (await call('/sales/products', { token: sari, branch: 'ALL' })).body;
        ok(stockCkr(prods2.find((p) => p.sku === 'BRG-1108')) === stock0 - 10, 'stok cabang berkurang saat faktur terbit', [stock0, stockCkr(prods2.find((p) => p.sku === 'BRG-1108'))]);
        ...
        const ckrBank = banks.find((b) => b.branchCode === 'CKR' && b.status === 'aktif' && b.bankName !== 'Kas').code;
        const jktBank = banks.find((b) => b.branchCode === 'JKT' && b.status === 'aktif').code;
        ...
        const revd = (cn2.body.journals ?? []).filter((j) => j.status === 'reversed').length;
        const revs = (cn2.body.journals ?? []).filter((j) => j.rule === 'REVERSAL').length;
        ...
        ok(stockCkr(prods3.find((p) => p.sku === 'BRG-1108')) === stock0 - 10, 'stok dikembalikan saat faktur dibatalkan');
        ...
        for (const b of ['CKR', 'ALL']) {
            ...
            const bad = rec.body.checks.filter((c) => !c.ok).map((c) => `${c.id}:${c.diff}`);
        ...
        ok(recAug.body.checks.find((c) => c.id === 'ar').ok, 'rekonsiliasi piutang Agu 2026 (data awal + penerimaan historis) cocok');
        const recv = await call('/sales/receivables', { token: andi, branch: 'ALL', period: '2026-08' });
        ok(recv.status === 200 && recv.body.kpi.reconciled && recv.body.aging.reduce((t, b) => t + b.value, 0) === recv.body.kpi.total, 'umur piutang = saldo 1-1200 dan jumlah ember = total', recv.body?.kpi);
        const tInv = await call('/sales/invoices', { token: taufik, branch: 'ALL' });
        ok(tInv.status === 200 && tInv.body.length > 0 && tInv.body.every((i) => i.branch === 'MDN'), 'manajer Medan hanya melihat faktur Medan (RLS)');
        ...
        ok(aud.status === 200 && ['invoice.created', 'invoice.issued', 'invoice.receipt'].every((x) => aud.body.data.some((a) => a.action === x)), 'siklus faktur tercatat di jejak audit', aud.body?.data?.map((a) => a.action));
