...
async function main() {
    ...
    const parent = accs.find((a) => a.code === '5-3000');
    ...
    const someDetail = accs.find((a) => a.type === 'detail' && a.category === 'Beban' && !a.isComputed);
    ...
    const used = accs.find((a) => a.type === 'detail' && a.category === 'Beban' && a.balance !== 0 && !a.isComputed);
    ...
    const jktBranch = (await call('/branches', { token: admin })).body.find((b) => b.code === 'JKT');
    ...
    ok(p1.body.data.length === 10 && p2.body.data.length === 10 && !p1.body.data.some((j) => p2.body.data.some((k) => k.id === j.id)) && p1.body.meta.total > 20, 'jurnal berpaginasi di server tanpa tumpang tindih', p1.body.meta);
    ...
    let ipSeq = 0;
    const authCall = (email, password) => call('/auth/login', { method: 'POST', headers: { 'X-Forwarded-For': `10.77.0.${(ipSeq += 1) % 250 + 1}` }, body: JSON.stringify({ email, password }) });
    ...
    const cookieOf = (r) => (r.headers.getSetCookie?.() ?? []).find((c) => c.startsWith('erp_refresh='))?.split(';')[0];
    ...
    ok(ulist.status === 200 && ulist.body.some((x) => x.email === 'andi@knm.co.id' && x.roles.length > 0) && !ulist.body.some((x) => x.email.startsWith('sistem@')), 'daftar pengguna memuat peran & menyembunyikan akun sistem', ulist.body?.length);
    ...
    for (let i = 0; i < 5; i += 1)
        await authCall(email, 'salah-salah-123');
    ...
    const lockedRow = (await call('/admin/users', { token: admin })).body.find((x) => x.id === cu.body.id);
    ...
    ok(roles.status === 200 && roles.body[0].code === 'admin' && roles.body.find((r) => r.code === 'admin').permissions.includes('admin.settings.manage'), 'daftar peran memuat izin & jumlah pengguna', roles.body?.[0]);
    ...
    ok(cat.status === 200 && cat.body.groups.flatMap((g) => g.items).length >= 28, 'katalog izin berlabel tersedia');
