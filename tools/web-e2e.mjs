...
const login = async (email) => {
  /* Batas laju login per IP: tiap login memakai alamat berbeda (API di belakang proksi tepercaya). */
  await page.setExtraHTTPHeaders({ 'X-Forwarded-For': `10.88.${Date.now() % 200}.${(ipSeq += 1) % 250 + 1}` });
...
await page.fill('[data-filter=user]', newEmail); await page.waitForSelector(`[data-user="${newEmail}"]`);
...
ok(await page.locator('[data-table=users] tbody tr[data-user]').count() >= 1, 'pencarian pengguna menyaring tabel');
...
await page.goto(base + '/pengguna'); await page.waitForSelector('[data-filter=user]'); await page.fill('[data-filter=user]', newEmail); await page.waitForSelector(`[data-user="${newEmail}"]`);
...
const salesCtx = async (branch) => {
  await page.click('.contextbar [data-action="switch-branch"]'); await page.click(`[data-set-branch="${branch}"]`); await page.waitForTimeout(300);
  await page.click('.contextbar [data-action="switch-period"]'); await page.click('[data-set-period="2026-09"]'); await page.waitForTimeout(300);
};
await logout(); await login('sari@knm.co.id'); await salesCtx('CKR');
await page.goto(base + '/pesanan-penjualan'); await page.waitForSelector('[data-table=orders] tbody tr[data-row]');
ok(await page.locator('[data-table=orders] tbody tr[data-row]').count() > 0, 'daftar pesanan penjualan dimuat'); await shot('40-pesanan');
await page.click('[data-action=new-order]'); await page.waitForSelector('#doc-customer option:nth-child(3)', { state: 'attached' });
const custVal = await page.locator('#doc-customer option', { hasText: 'PT Global Komponen Indo' }).getAttribute('value');
await page.selectOption('#doc-customer', custVal);
const braketVal = await page.locator('[data-line] [data-field=product] option', { hasText: 'BRG-1108' }).first().getAttribute('value');
await page.selectOption('[data-line] [data-field=product]', braketVal); await page.fill('[data-line] [data-field=qty]', '4');
await page.click('[data-action=add-line]'); await page.fill('[data-line] >> nth=1 >> [data-field=description]', 'Jasa pengiriman khusus'); await page.fill('[data-line] >> nth=1 >> [data-field=price]', '750000');
const expected = Math.round((4 * 385000 + 750000) * 1.11);
ok((await page.locator('[data-total]').innerText()).replace(/\D/g, '') === String(expected), 'pratinjau total (DPP + PPN 11%) dihitung di formulir', await page.locator('[data-total]').innerText());
await shot('41-pesanan-baru'); await page.click('[data-action=submit-order]');
await page.waitForSelector('.drawer [data-credit]'); await page.waitForTimeout(300);
const soNo = (await page.locator('.drawer .drawer-eyebrow .code').innerText()).trim();
ok(/^SO-2026-\d{4}$/.test(soNo) && (await page.locator('.drawer .drawer-eyebrow .pill').innerText()).includes('Disetujui'), 'pesanan diajukan & disetujui otomatis (lolos plafon)', soNo);
await page.click('[data-action=order-to-invoice]'); await page.waitForURL(/\/faktur\?id=/); await page.waitForSelector('.drawer [data-open]');
const invNo = (await page.locator('.drawer .drawer-eyebrow .code').innerText()).trim();
ok(/^INV-2026-\d{4}$/.test(invNo) && (await page.locator('.drawer .drawer-eyebrow .pill').innerText()).includes('Draf'), 'pesanan menjadi draf faktur', invNo);
ok(await page.locator('[data-action=issue-invoice]').count() === 0, 'pembuat faktur tidak ditawari tombol terbitkan (SoD)');
await shot('42-faktur-draf');
await page.keyboard.press('Escape'); await logout(); await login('andi@knm.co.id'); await salesCtx('CKR');
await page.goto(base + '/faktur'); await page.waitForSelector(`[data-invoice="${invNo}"]`); await page.click(`[data-invoice="${invNo}"]`);
await page.waitForSelector('[data-action=issue-invoice]'); await page.click('[data-action=issue-invoice]');
await page.waitForFunction(() => document.querySelector('.drawer .drawer-eyebrow .pill')?.textContent?.includes('Belum dibayar'), null, { timeout: 15000 });
ok(await page.locator('[data-table=invoice-journals] tbody tr').count() === 2, 'faktur terbit: jurnal penjualan & HPP tampil di laci'); await shot('43-faktur-terbit');
await page.keyboard.press('Escape'); await logout(); await login('sari@knm.co.id'); await salesCtx('CKR');
await page.goto(base + `/faktur`); await page.waitForSelector(`[data-invoice="${invNo}"]`); await page.click(`[data-invoice="${invNo}"]`);
await page.waitForSelector('[data-action=new-receipt]'); await page.click('[data-action=new-receipt]'); await page.waitForSelector('#rc-amount');
await page.fill('#rc-amount', '1000000'); await page.fill('#rc-ref', 'TRF-UJI'); await page.click('[data-action=save-receipt]');
await page.waitForFunction(() => document.querySelector('.drawer .drawer-eyebrow .pill')?.textContent?.includes('sebagian'), null, { timeout: 15000 });
ok((await page.locator('[data-table=receipts] tbody tr').count()) === 1, 'penerimaan sebagian tercatat'); await shot('44-penerimaan');
await page.keyboard.press('Escape');
await page.goto(base + '/piutang'); await page.waitForSelector('[data-aging]');
ok((await page.locator('[data-aging]').innerText()).includes('Cocok'), 'piutang usaha cocok dengan buku besar 1-1200'); await shot('45-piutang');
...
await page.goto(base + '/produk'); await page.waitForSelector('[data-product="BRG-1108"]');
ok((await page.locator('[data-product="BRG-1108"]').innerText()).includes('CKR'), 'produk menampilkan stok per cabang');
await page.goto(base + '/integrasi'); await page.waitForSelector('.pill');
ok(await page.locator('.pill[data-tone=danger]').count() === 0, 'integrasi Sep 2026 cocok setelah transaksi penjualan');
await logout(); await login('osmond@knm.co.id');
await page.goto(base + '/pesanan-penjualan'); await page.waitForSelector('[data-chip=menunggu]'); await page.click('[data-chip=menunggu]');
if (await page.locator('[data-table=orders] tbody tr[data-row]').count()) {
  await page.click('[data-table=orders] tbody tr[data-row] >> nth=0'); await page.waitForSelector('[data-decision]');
  ok(await page.locator('[data-action=approve-order]').count() === 1, 'manajer melihat panel keputusan & tombol setujui'); await shot('47-keputusan');
  await page.keyboard.press('Escape');
}
await logout(); await login('admin@knm.co.id');
