...
import { passwordProblems } from '@erp/domain';
...
export class AuthService {
  ...
  async login(email: string, password: string, meta: { ip?: string; userAgent?: string; requestId?: string }): Promise<LoginResult> {
    ...
    const outcome = await this.db.run(systemContext(), async (c): Promise<LoginResult | 'invalid' | 'locked'> => {
    ...
    if (outcome === 'invalid') throw generic();
    if (outcome === 'locked') throw new DomainError('ACCOUNT_LOCKED', `Akun terkunci sementara. Coba lagi setelah ${LOCK_MINUTES} menit.`, HttpStatus.LOCKED);
    return outcome;
  }
  ...
  async refresh(refreshToken: string, meta: { ip?: string; userAgent?: string; requestId?: string }): Promise<LoginResult> {
    ...
    const outcome = await this.db.run(systemContext(), async (c): Promise<LoginResult | { fail: string }> => {
      const { rows } = await c.query('SELECT * FROM sessions WHERE refresh_token_hash = $1', [this.sha256(refreshToken)]);
      const s = rows[0];
      if (!s) return { fail: 'Sesi tidak dikenal.' };
      if (s.revoked_at) {
      ...
      if (new Date(s.expires_at) < new Date()) return { fail: 'Sesi kedaluwarsa.' };
    ...
    if ('fail' in outcome) throw new UnauthorizedException(outcome.fail);
    return outcome;
  }
  ...
  async changeOwnPassword(user: RequestUser, current: string, next: string, meta: { ip?: string; userAgent?: string; requestId?: string }) {
    const problems = passwordProblems(next, user.email);
    if (problems.length) throw new DomainError('PASSWORD_WEAK', problems.join(' '), HttpStatus.UNPROCESSABLE_ENTITY, problems);
    if (current === next) throw new DomainError('PASSWORD_SAME', 'Kata sandi baru harus berbeda dari kata sandi lama.', HttpStatus.UNPROCESSABLE_ENTITY);
    return this.db.run(systemContext(), async (c) => {
      const u = (await c.query('SELECT password_hash FROM users WHERE id = $1', [user.id])).rows[0];
      if (!u?.password_hash || !(await argon2.verify(u.password_hash, current))) throw new DomainError('PASSWORD_WRONG', 'Kata sandi saat ini salah.', HttpStatus.UNPROCESSABLE_ENTITY);
      await c.query('UPDATE users SET password_hash = $2, must_change_password = false, password_changed_at = now(), failed_logins = 0, locked_until = NULL WHERE id = $1', [user.id, await AuthService.hashPassword(next)]);
